...
factor_to_ca0_score <- function(var, class, axes) {
  ...
  pc <- eigen_S$vectors
  X <- P %*% pc[, seq_len(nlambdas), drop=FALSE] |> as.data.frame() |> rownames_to_column("Var_Level") 
  score <- left_join(data.frame(Var_Level = var_levels), X, by = "Var_Level")
...
prepare_training_ca0 <- function(data, var_cols, class, axes=1) {
...
impute_score_ca0 <- function(var, extra) {
  ...
  X <- pluck(extra, "X")
  new_var_level_score <- X |> filter(Var_Level == "new") |> select(-Var_Level) |> as.list()
  test_score <- data.frame(Var_Level = var) |> left_join(X, by="Var_Level") |> replace_na(replace=new_var_level_score)
